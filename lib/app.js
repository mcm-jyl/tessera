...
module.exports = function(tilelive, options) {
  ...
  if (typeof options === "object") {
    ...
    var autoscale = ("autoscale" in options) ? options.autoscale : true;
if (options.sourceMaxZoom) {
  ...
  [2, 3].forEach(function(scale) {
    ...
    retinaURI.query.scale = autoscale ? scale : 1;
    // explicitly tell tilelive-mapnik to use larger tiles
    retinaURI.query.tileSize = retinaURI.query.scale * 256;
    ...
    var sourceURI = sourceURIs[scale],
        params = {
          tile: {
            ...
            y: y,
            format: format,
            retina: sourceURIs[scale].query.scale > 1,
            scale: sourceURIs[scale].query.scale
          }
        };
